...
| **Cline** | ✅ 対応済み（M3） | VSCode globalStorage `saoudrizwan.claude-dev/tasks/<id>/`（VS Code Server `~/.vscode-server/data/User/globalStorage/...` も追加で走査） |
...
# 単価表を更新して既存ログのコストを再計算したいときは全件再走査
uv run tokentracker ingest --full
...
**増分取り込み**: 既定では `ingest_state` に記録した各ファイルの `size`／`mtime` を照合し、
**前回から変更も追加もされていないファイルは開かずにスキップ**します。ログは追記式なので、
繰り返し実行しても**新規／変更分だけ**を収集し、実行のたびに「新規 N 件」を表示します。
単価表（pricing.toml）を更新して**既存ログのコストを再計算**したい場合だけ `--full` で全件を
再走査してください。
...
サイドバーの**「取り込み実行」ボタン**からその場で ingest（増分／全件）もでき、新規件数がその場で
表示されます。DB がまだ無い・空のときは、画面の案内からそのまま初回の取り込みを実行できます。
...
1h/5m 分離、`top-level == sum(iterations)`、未割当コストの別計上、TZ バケット、**増分取り込みの
スキップ／再取り込み**、ダッシュボードの **Streamlit AppTest**）を `tests/` に固定しています。

```bash
uv run pytest                                   # コアのみ
uv run --extra dashboard --extra dev pytest     # ダッシュボード(AppTest)も含めて全件
```
